<%@taglib prefix="s" uri="/struts-tags" %>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<head>
    <link rel="stylesheet" type="text/css" href="/resources/css/theme_styles.min.css" />
    <link rel="stylesheet" type="text/css" href="/vendors/bootstrap/bootstrap.min-3.3.7.css" />
    
    <title>Solicitud de Transporte</title>
</head>

<body>
    
    <div class="container">
        <h1>Imprimir</h1>
        <div class="row ">
            <div class="col-sm-12">
                <div class="main-box clearfix">
                    <p>Estamos preparando tu documento</p>
                </div>
            </div>
        </div>
    </div>
    
    <script src="/vendors/jQuery/jquery-3.1.1.min.js"></script>
    <script src="/vendors/bootstrap/bootstrap-3.3.7.min.js"></script>
    <script src="/resources/js/scripts.min.js" type="text/javascript"></script>
    <script type="text/javascript" src="/vendors/pdfmake-browserified/pdfmake-browserified.min.js"></script>
    <script src="/vendors/nanoscroller/jquery.nanoscroller-0.8.7.min.js"></script>        
    
    <script type="text/javascript" language="javascript" class="init">
        $(document).ready(function () {
            openPdf();
            setTimeout(function () {
                window.parent.$.fancybox.close();
            }, 10);
        });
        
        function openPdf() {
            var docDefinition = {
                pageOrientation: 'landscape',
                pageMargins: [40, 100, 40, 20],
                header: function () {
                    return {
                        columns: [
                            {
                                margin: [25, 10, 0, 0],
                                image: 'data:image/jpeg;base64,',
                                height: 90,
                                width: 85
                            },
                            {
                                margin: [160, 25, 160, 0],
                                width: 'auto',
                                text: 'INSTITUTO POLITÉCNICO NACIONAL\nSecretaría de Servicios Educativos\nDirección de Servicios Estudiantiles\n\nFORMATO DE REPORTE DE INGRESOS Y EGRESOS',
                                alignment: 'center'

                            },
                            {
                                margin: [0, 10, 0, 0],
                                image: 'data:image/jpeg;base64,',
                                height: 90,
                                width: 95
                            }
                        ],
                        columnGap: 15
                    };
                },
                footer: function () {
                    return {
                        columns: [
                            {text: 'Fecha y hora de impresion: <s:date name="hoy" format="dd/MM/yyyy HH:mm:ss" />', italics: true, fontSize: 9, alignment: 'right'},
                        ],
                        margin: [45, 0]
                    };
                },
                content: [
                    {
                        table: {
                            headerRows: 0,
                            widths: [50, 200, 100, '*'],
                            body: [
                                [{text: 'FECHA: ', style: 'textoCentro'},
                                    {text: '<s:property value="fecha" escape="false"/>', style: 'texto'},
                                    {text: 'PROGRAMA DE BECA: ', style: 'textoCentro'},
                                    {text: '<s:property value="beca" escape="false"/>', style: 'texto'}]
                            ]
                        },
                        layout: 'noBorders'
                    },
                    {
                        table: {
                            headerRows: 0,
                            widths: ['*', '*', '*'],
                            body: [
                                [{text: '<s:property value="alumno.apellidoPaterno" escape="false"/>', style: 'textoCentro', bold: false},
                                    {text: '<s:property value="alumno.apellidoMaterno" escape="false"/>', style: 'textoCentro', bold: false},
                                    {text: '<s:property value="alumno.nombre" escape="false"/>', style: 'textoCentro', bold: false}]
                            ]
                        },
                    },
                    {
                        table: {
                            headerRows: 0,
                            widths: ['*', '*', '*'],
                            body: [
                                [{text: 'APELLIDO PATERNO', style: 'textoCentro'},
                                    {text: 'APELLIDO MATERNO', style: 'textoCentro'},
                                    {text: 'NOMBRE (S)', style: 'textoCentro'}]
                            ]
                        },
                        layout: 'noBorders'
                    },
                    {
                        table: {
                            headerRows: 0,
                            widths: [400, '*', '*'],
                            body: [
                                [{text: ' <s:property value="alumno.datosAcademicos.unidadAcademica.nombre" escape="false"/>', style: 'textoCentro', bold: false},
                                    {text: ' ', style: 'textoCentro'},
                                    {text: ' <s:property value="alumno.celular" escape="false"/>', style: 'textoCentro', bold: false}]
                            ]
                        },
                    },
                    {
                        table: {
                            headerRows: 0,
                            widths: [400, '*', '*'],
                            body: [
                                [{text: 'UNIDAD ACADÉMICA', style: 'textoCentro'},
                                    {text: 'TELÉFONO CON CLAVE LADA', style: 'textoCentro'},
                                    {text: 'TELÉFONO CELULAR', style: 'textoCentro'}]
                            ]
                        },
                        layout: 'noBorders'
                    },
                    {
                        table: {
                            headerRows: 1,
                            widths: [300, 120, 40, 70, 120, '*'],
                            body: [
                                [{text: 'DATOS DEL INGRESO FAMILIAR', colSpan: 6, style: 'textoCentro'},
                                    {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}],
                                [{text: 'NOMBRE', style: 'textoCentro'},
                                    {text: 'PARENTESCO', style: 'textoCentro'},
                                    {text: 'EDAD', style: 'textoCentro'},
                                    {text: 'ESTADO CIVIL', style: 'textoCentro'},
                                    {text: 'INGRESOS MENSUALES', style: 'textoCentro'},
                                    {text: 'OCUPACIÓN', style: 'textoCentro'}],
                                [{text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}],
                                [{text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}],
                                [{text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}],
                                [{text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}],
                                [{text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}],
                                [{text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}],
                                [{text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}, {text: ' '}],
                                [{text: 'TOTAL INGRESO MENSUAL FAMILIAR', style: 'textoCentro'},
                                    {text: ' ', colSpan: 5, style: 'textoCentro'}]
                            ]
                        },
                    },
                    {text: ' ', style: 'leyenda'},
                    {
                        columns: [
                            {
                                width: 500,
                                table: {
                                    headerRows: 0,
                                    widths: ['*', '*', '*', '*'],
                                    body: [
                                        [{text: 'DATOS DEL EGRESO FAMILIAR', colSpan: 4, style: 'textoCentro'},
                                            {text: ' '}, {text: ' '}, {text: ' '}],
                                        [{text: 'CONCEPTO', style: 'textoCentro'},
                                            {text: 'MONTO', style: 'textoCentro'},
                                            {text: 'CONCEPTO', style: 'textoCentro'},
                                            {text: 'MONTO', style: 'textoCentro'}],
                                        [{text: 'RENTA Y/O HIPOTECA', style: 'textoCentro'}, {text: ' '},
                                            {text: 'GAS', style: 'textoCentro'}, {text: ' '}],
                                        [{text: 'LUZ', style: 'textoCentro'}, {text: ' '},
                                            {text: 'ALIMENTACIÓN', style: 'textoCentro'}, {text: ' '}],
                                        [{text: 'AGUA', style: 'textoCentro'}, {text: ' '},
                                            {text: 'TRANSPORTACIÓN', style: 'textoCentro'}, {text: ' '}],
                                        [{text: 'PREDIAL', style: 'textoCentro'}, {text: ' '},
                                            {text: 'CELULARES', style: 'textoCentro'}, {text: ' '}],
                                        [{text: 'TELÉFONO', style: 'textoCentro'}, {text: ' '},
                                            {text: 'OTRO TIPO DE EGRESOS', style: 'textoCentro'}, {text: ' '}],
                                        [{text: 'TOTAL EGRESO MENSUAL FAMILIAR', colSpan: 2, style: 'textoCentro'},
                                            {text: ' '}, {text: ' ', colSpan: 2, style: 'textoCentro'}, {text: ' '}, ]
                                    ]
                                },
                            },
                            {
                                width: 50,
                                text: ' '
                            },
                            {
                                width: 200,
                                style: 'leyenda',
                                text: '\nBajo protesta de decir verdad, el alumno manifiesta que la información contenida en este formato ' +
                                        'es verídica, y que no es beneficiario de ningún otro programa de apoyo económico otorgado ' +
                                        'para su educación. En caso de ser necesario el (los) integrante(s) del Subcomité de Becas de ' +
                                        'la Unidad Académica, podrán solicitar la documentación comprobatoria que ampare lo declarado ' +
                                        'en el presente documento; de comprobarse falsedad en los datos aqui presentados, se procederá ' +
                                        'a la aplicación de lo establecido en el Reglamento de Becas para alumnos del Instituto ' +
                                        'Politécnico Nacional, así como en las Bases y Lineamientos para la Operación de los Programas ' +
                                        'de Becas para el Nivel Medio Superior y Superior, las Reglas de Operación de cada Programa de ' +
                                        'Becas y/o lo establecido en el propio Programa.'

                            }
                        ]
                    },
                    {text: ' ', style: 'leyenda'},
                    {
                        table: {
                            headerRows: 0,
                            widths: ['*', '*', '*', 100],
                            body: [
                                [{text: '\n\n\n\n <s:property value="alumno.nombre" escape="false"/>' +
                                                ' <s:property value="alumno.apellidoPaterno" escape="false"/>' +
                                                ' <s:property value="alumno.apellidoMaterno" escape="false"/>', style: 'leyenda'},
                                    {text: '\n\n\n\nNOMBRE Y FIRMA DEL PADRE, MADRE O TUTOR', style: 'leyenda'},
                                    {text: '\n\n\n\nNOMBRE Y FIRMA DE QUIEN RECIBE LA INFORMACIÓN', style: 'leyenda'},
                                    {text: '\n\n\n\nSELLO DE LA UA', style: 'leyenda'}]
                            ]
                        },
                    },
                ],
                styles: {
                    headerTable: {
                        fontSize: 10,
                        bold: true,
                    },
                    texto: {
                        fontSize: 9,
                        alignment: 'justify',
                    },
                    textoCentro: {
                        fontSize: 9,
                        alignment: 'center',
                        bold: true
                    },
                    leyenda: {
                        fontSize: 7.5,
                        alignment: 'center'
                    }
                }
            };
            createPdf(docDefinition).open();
        }
    </script>
</body>